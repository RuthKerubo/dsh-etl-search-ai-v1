...
<template>
  <!-- Hero -->
  <section class="bg-gradient-to-br from-indigo-700 via-indigo-600 to-violet-600 text-white">
    <div class="max-w-4xl mx-auto px-4 py-20 text-center">
      <span class="inline-block mb-4 px-3 py-1 bg-white/20 rounded-full text-sm font-medium tracking-wide">
      ...
      <form @submit.prevent="search" class="flex max-w-2xl mx-auto shadow-xl rounded-xl overflow-hidden">
        <input v-model="query" type="text"
          placeholder="e.g. river water quality nitrate, soil carbon England..."
          class="flex-1 px-5 py-4 text-gray-900 text-base focus:outline-none" autofocus />
        <button type="submit"
          class="bg-indigo-900 text-white px-6 py-4 font-semibold hover:bg-indigo-950 transition-colors shrink-0">
          Search
        </button>
      </form>
      ...
      <div class="mt-4 flex flex-wrap justify-center gap-2">
        <span class="text-indigo-200 text-sm">Try:</span>
        <button v-for="ex in ['climate projections', 'pollinator abundance', 'peatland carbon flux', 'groundwater levels']"
          :key="ex"
          @click="query = ex; search()"
          class="text-sm text-indigo-100 underline underline-offset-2 hover:text-white transition-colors">
          {{ ex }}
        </button>
      </div>
    </div>
  ...
  <section class="bg-white border-b border-gray-100">
    <div class="max-w-4xl mx-auto px-4 py-8 grid grid-cols-2 sm:grid-cols-4 gap-6 text-center">
      ...
      <div>
        <div class="text-3xl font-bold text-indigo-600">384</div>
  ...
  <section class="max-w-5xl mx-auto px-4 py-16">
    <h2 class="text-2xl font-bold text-gray-900 text-center mb-10">Platform Features</h2>
    ...
    <div class="max-w-4xl mx-auto px-4 py-16">
      ...
      <div class="grid sm:grid-cols-3 gap-8">
        <div v-for="s in steps" :key="s.n" class="text-center">
          <div
            class="w-10 h-10 rounded-full bg-indigo-600 text-white font-bold text-lg flex items-center justify-center mx-auto mb-4">
            {{ s.n }}
          </div>
